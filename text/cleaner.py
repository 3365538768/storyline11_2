...
def clean_text(text, language, version=None):
    ...
    if hasattr(language_module,"text_normalize"):  #hasattr(object, name) 是一个内置函数，用于检查对象 object 是否具有名称为 name 的属性或方法。它返回 True 或 False。
        norm_text = language_module.text_normalize(text)
    else:
        norm_text=text
    if language == "zh" or language=="yue":##########
        phones, word2ph = language_module.g2p(norm_text) #g2p 通常是指 "grapheme-to-phoneme" 转换，即将字素（文字）转换为音素（发音单元）。在语音合成或语音处理领域，这种转换很常见。
        assert len(phones) == sum(word2ph)
        assert len(norm_text) == len(word2ph)
    elif language == "en":
        phones = language_module.g2p(norm_text) #phones就是字/词对应的音素，而word2ph是中文独有的，因为一个字对应多个音素
        if len(phones) < 4:
